...
namespace TreeGP.AlgorithmModels.PopInit
{
    ...
    /// <summary>
    /// Population Initialization method following the "Ramped Full" method described in Section 2.2 of "A Field Guide to Genetic Programming"
    /// </summary>
    ...
    public class TGPPopInitInstruction_RampedFull<P, S> : PopInitInstruction<P, S>
        where S : IGPSolution
        where P : IGPPop
    {
        public TGPPopInitInstruction_RampedFull()
        {

        }

        public TGPPopInitInstruction_RampedFull(XmlElement xml_level1)
            : base(xml_level1)
        {
        ...
        public override void Initialize(P pop)
        {
            ...
            for (int i = 0; i < part_count; i++)
            {
                for (int j = 0; j < interval; ++j)
                {
                    ...
                    program.CreateWithDepth(i + 1, TGPProgram.INITIALIZATION_METHOD_FULL);
            ...
            {
                S program = (S)pop.CreateSolution();
                program.CreateWithDepth(iMaximumDepthForCreation, TGPProgram.INITIALIZATION_METHOD_FULL);
                pop.AddSolution(program);
            }
        ...
        {
            TGPPopInitInstruction_RampedFull<P, S> clone = new TGPPopInitInstruction_RampedFull<P, S>();
            return clone;
        }
        ...
        public override string ToString()
        {
            ...
            sb.Append(">> Name: TGPPopInitInstruction_RampedFull\n");
            return sb.ToString();
        }
    }
}
